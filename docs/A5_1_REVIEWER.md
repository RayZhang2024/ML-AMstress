...
stdin (`codex exec --model gpt-5.6-terra --sandbox read-only -c
model_reasoning_effort="high" -c approval_policy="never" --output-last-message
<temporary-final-message> -`). Codex progress/session
...
removed. The explicit model and reasoning effort prevent account or Codex
defaults from selecting a different reviewer profile when the trusted runner
remains pinned to Codex CLI 0.149.1. Its child environment excludes `GITHUB_TOKEN`, `GH_TOKEN`,
