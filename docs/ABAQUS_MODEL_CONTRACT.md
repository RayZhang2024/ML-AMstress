...
6. Optional heat treatment follows those removals. `apply_materials.py` creates the CAE heat-treatment step at `N + 4 + rem_layers` with a normalized `timePeriod=1.0`, `initialInc=maxInc=0.05`, `maxNumInc=10000`, and `minInc=0.0002`, so the existing UTEMP ramp/hold/cool profile can reach its `TIME(1)` transitions. The same settings are applied to the final step in the supported `BStep-*` fallback. When Input & UTEMP heat treatment is enabled, `create_input.py` resolves the final distinct post-removal CAE step from ModelChange history and emits that step index in UTEMP; it does not add or inject a duplicate bottom-removal setting. If no distinct compatible CAE heat-treatment step exists, the pre-input gate fails before output generation. The Build Model and Input & UTEMP heat-treatment controls remain independent.
