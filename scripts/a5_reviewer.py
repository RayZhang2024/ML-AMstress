...
CONTROL_PLANE_REQUIREMENT_RE = re.compile(
    ...
    r"worker\b.*\bclaims?\b.*\bfresh\s+deterministic\s+green\s+branch\b.*\bcompletes?\s+successfully\b.*\bfirst\s+execution\s+attempt\b|"
    r"live\s+worker\s+evidence\b.*\bno\s+evidence\s+of\s+fallback\b.*\b(?:implementation\s+)?model\b|"
    r"normal\s+worker\s+validation\s+passes?\b|"
    r"a5\s+review\s+is\s+clean\b.*\b(?:same|exact)\s+head\b|"
...
def reviewer_command(resolved_executable: str, final_output_path: str) -> list[str]:
    ...
    return [
        resolved_executable, "exec", "--model", REVIEWER_MODEL, "--sandbox", "read-only", "-c",
        'model_reasoning_effort="%s"' % REVIEWER_REASONING_EFFORT, "-c", 'approval_policy="never"',
